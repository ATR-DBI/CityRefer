...
class Solver():
    ...
    def _feed(self, dataloader, phase, epoch_id):
        ...
        for data_dict in dataloader:
            ...
            data_dict['input_ids'] = inputs.cuda()
            data_dict['labels'] = labels.cuda()
            data_dict['attention_mask'] = encoded_query["attention_mask"].cuda() #to(device) 
            data_dict['geo_mask'] = get_mask(data_dict['geo_len'], self.max_num_object).cuda() #to(device) 
            
            # landmark
            if 'landmark_len' in data_dict:
                landmark_texts = sum(data_dict["landmark_texts"], [])
                encoded_landmark = self.tokenizer(
                    landmark_texts,
                    add_special_tokens=True,
                    max_length=self.max_land_len,
                    padding="longest",
                    truncation=True,
                    return_tensors="pt",
                )
                ...
                data_dict['landmark_mask'] = get_mask(data_dict['landmark_len'], self.max_num_landmark).cuda() # to(device)
            ...
            self.log[phase]["mlm_loss"].append(self._running_log["mlm_loss"].item())
        ...
        if phase == "val":
            ...
            if cur_best > self.best[cur_criterion]:
                ...
                self.best["mlm_loss"] = np.mean(self.log[phase]["mlm_loss"])
    ...
    def _dump_log(self, phase):
        log = {
            "loss": ["loss", "ref_loss", "lang_loss", "mlm_loss"],
    ...
    def _finish(self, epoch_id):
        ...
        for phase in ["train", "val"]:
            self._log_writer[phase].export_scalars_to_json(
                os.path.join(self.CONF.PATH.OUTPUT, self.stamp, "tensorboard/{}".format(phase), "all_scalars.json"))
    ...
    def _train_report(self, epoch_id):
        ...
        iter_report = self.__iter_report_template.format(
            ...
            train_mlm_loss=round(np.mean([v for v in self.log["train"]["mlm_loss"]]), 5),
    ...
    def _epoch_report(self, epoch_id):
        ...
        epoch_report = self.__epoch_report_template.format(
            ...
            val_mlm_loss=round(np.mean([v for v in self.log["val"]["mlm_loss"]]), 5),
    ...
    def _best_report(self):
        ...
        best_report = self.__best_report_template.format(
            ...
            mlm_loss=round(self.best["mlm_loss"], 5),
